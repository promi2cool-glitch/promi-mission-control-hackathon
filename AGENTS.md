...
`.xo/` is created **by XO Space**, not by you, the moment this folder is created, cloned, or first seen in the projects root: `.xo/project.json` already carries identity (pid, name, owner, created_at), and `todos.json`, `workitems.json` and `peers.json` start empty. If `.xo/project.json` is still missing or marked `_template: true`, the service has not reached this folder yet: read identity from the harness env and carry on. Either way, **don't edit anything in `.xo/`**.
...
## 12. Project-specific rules (Promi Mission Control)

This repo is public and is the hackathon-safe integration layer only — not the full Promi system. In addition to everything above:

- **No secret exposure.** Never commit, log, print, or echo API keys, tokens, or credentials. Scan staged files before every commit.
- **No production deployment.** This project never deploys to a production environment.
- **No live trading.** Nothing here executes real trades or connects to live trading systems, ever.
- **No destructive filesystem operations outside this project folder.** Never touch files outside this project's own tree.
- **No automatic Git merge.** Merges are reviewed and performed deliberately, never on autopilot.
- **Evidence required before claiming success.** A claim of "done" must be backed by something checkable (test output, a diff, a run log) — not a self-report.
- **Worker completion is not equivalent to a verification pass.** The verifier (O3) is the actual trust boundary; a worker finishing is not the same as the result being correct.
- **Use official XO APIs where possible.** Prefer the documented `/api/*` surface over ad-hoc filesystem or shell workarounds, per the `xo-projects` skill's hard constraints.
- **Build/test/fix loop.** Prefer small, verifiable iterations (build → test → fix) over large unverified changes.
